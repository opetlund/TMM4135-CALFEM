...
def make_B_matrix(Nmatrix, number_of_nodes):
    ...
    for i in range(number_of_nodes):
        ...
        Bmatrix[:, i * 2 +
                1] = np.array([[0], [Nmatrix[1, i]], [Nmatrix[0, i]]])
    return Bmatrix
...
def quad4e(ex, ey, D, thickness, eq=None):
    ...
    # Solves for K and fe at all integration points
    for iGauss in range(numGaussPoints):
        for jGauss in range(numGaussPoints):
            ...
            # Collect global x- and y coordinates in one matrix
            H = np.transpose([ex, ey])
            # Collect gradients of shape function evaluated at xi and eta
